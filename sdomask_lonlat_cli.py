...
def main(startend, odir, tsdo=None, glonlim=[-180,180], glatlim=[-90,90], alt_km=0, 
         wl=193, dlon=None, dlat=None, dt=10, 
         aiafolder=None,
         ):
    ...
    for it,T in enumerate(times):
        ...
        C = ix[0].size
        print ("Eclipse possible in {} pixels".format(C))
    #    for k in range(C):
    ...
    #    of_temp = np.nan*np.ones((glon.size, glat.size))
        for i in range(of_worker.size):
            of[ix[0][i],ix[1][i]] = of_worker[i].result()
    #        of = utils.mask_lonlat_sdo(SDO, T, glon, glat, ghgt, wl=wl, verbose=False)
        # TO XARRAY
        X = xarray.Dataset(
            {
                "of": (("glat", "glon"), of),
...
if __name__ == '__main__':
    ...
    main(P.startend, odir=P.odir, dt = P.tres, tsdo = P.tsdo,
         ...
         wl = P.wl, alt_km= P.altkm, aiafolder = P.sdodir)
